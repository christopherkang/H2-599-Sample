    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var qsim = new QuantumSimulator())
            {
                ...
                var H2EstimateCompiledEnergyRPE = qsim.Get<H2EstimateCompiledEnergyRPE, H2EstimateCompiledEnergyRPE>();
                ...
                Func<int, Double> estAtBondLength = (idx) => Enumerable.Average(
                    from idxRep in Enumerable.Range(0, 10)
                    select H2EstimateEnergyRPE.Body.Invoke((idx, 6, 1.0))
                );

                Func<int, Double> estAtBondLengthCompiled = (idx) => Enumerable.Average(
                    from idxRep in Enumerable.Range(0, 10)
                    select H2EstimateCompiledEnergyRPE.Body.Invoke((idx, 6, 1.0))
                );
                ...
                foreach (var idxBond in Enumerable.Range(0, 54))
                {
                    ...
                    {
                        var est = H2EstimateEnergyRPE.Body.Invoke((idxBond, 6, 1.0));
                        System.Console.WriteLine($"\tEst: {est}");

                        // H2EstimateEnergyRPE.Run(estimator, idxBond, 6, 1.0).Wait();
                    }

                    foreach (var _ in Enumerable.Range(0, 10))
                    {
                        var estCompiled = H2EstimateCompiledEnergyRPE.Body.Invoke((idxBond, 6, 1.0));
                        System.Console.WriteLine($"\tCompEst: {estCompiled}");

                        // H2EstimateCompiledEnergyRPE.Run(estimator, idxBond, 6, 1.0).Wait();
                    }
